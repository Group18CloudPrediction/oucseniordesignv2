...
class Map extends Component {
    constructor(props) {
        ...
        this.callAPI();
        if (latLongs["sub" + this.props.stationID]) {
            console.log("This is sub " + this.props.stationID);
            subscribeToCoverageN(this.props.stationID, (err, coverage_img) => {

                // If already exists, update the coverage image

                // If Coverage Overlay is available, recompute the bounds given new CBH
                console.log("subscribe to coverage " + this.props.stationID);
            ...
            subscribeToShadowN(this.props.stationID, (err, shadow_img) => {
    ...
    componentDidMount() {
        ...
        if (latLongs["sub" + this.props.stationID]) {
            CENTER = latLongs["sub" + this.props.stationID];
        } else {
            CENTER = latLongs["CENTER"];
        }
        ...
        this.shadowOverlay = L.imageOverlay('', [
        ...
        this.coverageBorder = L.rectangle([
            [28.42000000001, -81.42000000001],
            [28.42000000002, -81.42000000002]
