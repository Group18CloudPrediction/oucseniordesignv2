...
class CoverageMap extends Component {
  ...
  render() {
    return <div id="map" style={{ display: "flex", height: "550px" }}></div>;
  }
  ...
  addDistanceToCoordinate(
    ...
    finalCoordinate[0] =
      startCoordinate[0] + (EWdistance + NSoffset) / 362775.6;
    finalCoordinate[1] =
      startCoordinate[1] +
      ((NSdistance + EWoffset) / 365223.1) *
        Math.cos((startCoordinate[0] * Math.PI) / 180);
  ...
  getImageBounds(isShadow) {
    ...
    var smallHypo = Math.sqrt(
      Math.pow(CALIB[0] / 2, 2) + Math.pow(CALIB[1] / 2, 2)
    );
    ...
    var upperLeftCorner = this.addDistanceToCoordinate(
      CENTER,
      -NSdistance,
      EWdistance,
      sun.altitude,
    ...
    var bottomRightCorner = this.addDistanceToCoordinate(
      CENTER,
      NSdistance,
      -EWdistance,
      sun.altitude,
